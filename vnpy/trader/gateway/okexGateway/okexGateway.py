            ...
            if str(orderinfo.exchangeOrderID) in self.sendOrderDict.keys():
                ...
                if order.status in [STATUS_ALLTRADED, STATUS_CANCELLED]:
                        ...
                        if len(self.filledList)>0:
                            for exid in self.filledList:
                                if exid in self.sendOrderDict.keys():
                                    del self.sendOrderDict[exid]
                                if exid in self.wsOrderDict.keys():
                                    del self.wsOrderDict[exid]
                                if exid in self.exchangeOrderDict.keys():
                                    del self.exchangeOrderDict[exid]
                    # else:
                    #     self.filledList.append(str(order.exchangeOrderID))
